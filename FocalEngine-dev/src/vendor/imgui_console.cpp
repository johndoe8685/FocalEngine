...
ImGuiConsole::ImGuiConsole(std::string c_name, size_t inputBufferSize) 
    : m_ConsoleName(std::move(c_name)), NixTools::System("Default", "Console")
{
    ...
    //Additional settings that is written by johndoe8685
    m_shouldShowDemoWindow = false;
    m_shouldShowCheckstyle = false;
    m_changeVsyncVariable = true;
...
void ImGuiConsole::Draw(bool* p_open)
{
    ...
    //Render windows if there is window that is called from the console written by johndoe8685

    if (m_shouldShowDemoWindow)
    {
        ImGui::ShowDemoWindow();
    }

    if (m_shouldShowCheckstyle)
    {
        ImGui::Begin("Show Style Editor");
...
void ImGuiConsole::RegisterConsoleCommands()
{
    ...
    m_ConsoleSystem.RegisterCommand("showDemoWindow", "Show ImGUI Demo Window", [this]()
    {
            m_shouldShowDemoWindow = !m_shouldShowDemoWindow;
    });
    
    m_ConsoleSystem.RegisterCommand("showImGUIStyleEditor", "Show ImGUI Style Editor", [this]()
    {
        m_shouldShowCheckstyle = !m_shouldShowCheckstyle;
    });

    m_ConsoleSystem.RegisterCommand("firstperson", "Change camera to FPS Camera", [this]() {
        m_ConsoleSystem.Log() << "Changed camera to FPS camera" << csys::endl;
    });

    m_ConsoleSystem.RegisterCommand("vramusage", "Show Vram Usage", [this]() {
        int vram[4];
        vram[0] = 0;
        vram[1] = 0;
        vram[2] = 0;
...
#define GL_GPU_MEMORY_INFO_EVICTED_MEMORY_NVX 0x904B
        ...
        m_ConsoleSystem.Log() << "Available Vram (kb): " << vram[0] << csys::endl;
        });

    m_ConsoleSystem.RegisterCommand("freeroam", "Change camera to FreeRoam Camera", [this]() {
        m_ConsoleSystem.Log() << "Changed camera to FreeRoam camera" << csys::endl;
        });

    m_ConsoleSystem.RegisterCommand("msg", "Message command", [this](const csys::String& text) {
        debugger.giveMessage(NixTools::Info, text.m_String);
        m_ConsoleSystem.Log() << text.m_String << csys::endl;
    }, csys::Arg<csys::String>("text_str"));

    m_ConsoleSystem.RegisterCommand("vsync", "Change VSync setting", [this](const int interval) 
    {
        glfwSwapInterval(interval);
            /*
        if (m_changeVsyncVariable) glfwSwapInterval(0);
        else glfwSwapInterval(1);
        m_changeVsyncVariable = !m_changeVsyncVariable;*/
    }, csys::Arg<int>("interval_val"));

    m_ConsoleSystem.RegisterCommand("quit", "Quit from NixEngine", [this]()
    {
            m_shouldExit = true;
    });
}
